...
<body>
  <header>
    ...
    <p>JWT token'ınızı girip ürün / kategori CRUD işlemlerini doğrudan veritabanına yazın.</p>
  </header>
  ...
  "password": "demo"
          ...
          <small>Yukarıdaki gövde ile <code>/admin/login</code> isteğini gönderip token alın ve aşağıya yapıştırın.</small>
        </div>
        <div class="grid">
          <label>Admin JWT Token
            <input id="token" name="token" placeholder="Bearer &lt;token&gt;" autocomplete="off" />
          </label>
          <small class="muted">Token sizin tarafınızdan girilir, formlar gönderilirken header olarak kullanılır.</small>
        </div>
      </div>
    </section>
  ...
  <script>
    ...
    categoryForm?.addEventListener('submit', async (e) => {
      ...
      try {
        const result = await postJSON('/admin/categories', payload);
    ...
    productForm?.addEventListener('submit', async (e) => {
      ...
      try {
        const result = await postJSON('/admin/products', payload);
